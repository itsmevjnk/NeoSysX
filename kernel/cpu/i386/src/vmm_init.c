...
void vmm_lh_init(void) {
    ...
    vmm_info_t* vmm_info = (vmm_info_t*)((uintptr_t)&vmm_kernel_struct - 0xC0000000);
    ...
    vmm_info->cr3 = (uintptr_t)&vmm_kernel_struct.pd - 0xC0000000; // populate CR3
}

void vmm_reserve_initial_pt(void) {
    for (size_t pde = (0xC0000000 >> 22); pde < 1024; pde++) {
        if (vmm_kernel_struct.pt[pde])
            pmm_reserve((uintptr_t)vmm_kernel_struct.pt[pde] - 0xC0000000, 4096);
    }
}