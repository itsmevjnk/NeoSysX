...
#define LOG_INFO_HEADER "\x1B[102;30mI\x1B[49;92m"
#define LOG_INFO(fmt, ...) do { \
    uint64_t __ts = timer_get(); \
    printf(LOG_INFO_HEADER " %llu.%06u\x1B[0m " fmt "\n", __ts / TIMER_TICK_1_SEC, (unsigned)(__ts % TIMER_TICK_1_SEC) / 1000, ##__VA_ARGS__); \
} while(0)

#define LOG_WARN_HEADER "\x1B[103;30mW\x1B[49;93m"
#define LOG_WARN(fmt, ...) do { \
    uint64_t __ts = timer_get(); \
    printf(LOG_WARN_HEADER " %llu.%06u\x1B[0m " fmt "\n", __ts / TIMER_TICK_1_SEC, (unsigned)(__ts % TIMER_TICK_1_SEC) / 1000, ##__VA_ARGS__); \
} while(0)

#define LOG_ERROR_HEADER "\x1B[101;30mE\x1B[49;91m"
#define LOG_ERROR(fmt, ...) do { \
    uint64_t __ts = timer_get(); \
    printf(LOG_ERROR_HEADER " %llu.%06u\x1B[0m " fmt "\n", __ts / TIMER_TICK_1_SEC, (unsigned)(__ts % TIMER_TICK_1_SEC) / 1000, ##__VA_ARGS__); \
} while(0)

#define LOG_DEBUG_HEADER "\x1B[105;30mD\x1B[49;95m"
#define LOG_DEBUG(fmt, ...) do { \
    uint64_t __ts = timer_get(); \
    printf(LOG_DEBUG_HEADER " %llu.%06u\x1B[0m " fmt "\n", __ts / TIMER_TICK_1_SEC, (unsigned)(__ts % TIMER_TICK_1_SEC) / 1000, ##__VA_ARGS__); \
} while(0)
